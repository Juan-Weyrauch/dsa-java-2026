package ucu.edu.aed.tda.generic_trie;


import java.util.List;
import java.util.function.Consumer;

public interface TINodoGenerico<T extends Comparable<T>> {
    T getDato();

    /**
     * Agrega un nodo con dato T al padre
     */
    boolean agregarHijo(T padre, T hijo);

    /**
     * elimina el nodo y sus hijos directos utilizando el criterio pasado
     */
    TINodoGenerico<T> eliminar(Comparable<T> criterio);

    /**
     * busca un nodo utilizando el criterio pasado
     */
    TINodoGenerico<T> buscar(Comparable<T> criterio);

    /**
     * obtiene al nodo padre del hijo que satisface el criterio pasado
     */
    TINodoGenerico<T> obtenerPadre(Comparable<T> criterio);

    void preOrden(Consumer<TINodoGenerico<T>> consumidor);

    void inOrden(Consumer<TINodoGenerico<T>> consumidor);

    void postOrden(Consumer<TINodoGenerico<T>> consumidor);

    int altura();

    /**
     * retorna la cantidad de hijos que tiene el nodo actual
     */
    int grado();

    void vaciar();

    /**
     * retorna los hijos directos de este nodo
     */
    List<T> obtenerHijos();
}
